...
static void SetRuntimePerformancePreference(BOOL enabled) {
    NSMutableDictionary *prefs=CPUthermalReadMutablePrefs()?:[NSMutableDictionary dictionary];
    prefs[S("powerMode")]=S("fullPower"); prefs[S("highPerformanceModeEnabled")]=@(enabled); CPUthermalWritePrefs(prefs);
    CPUthermalPostPowerMode(S("fullPower")); notify_post(kCPUthermalSettingsChangedNotifC);
}
static int ApplyMountedPerformancePatch(void) {
    ...
    MakeStorageWritable(storage); SetRuntimePerformancePreference(YES); return RestartThermalMonitor();
}
...
static int RestoreThermalSchedule(void){NSString*d=SchedulerBackupRoot(),*t=StoragePath(S("/System/Library/ThermalMonitor")),*m=ModelFile(t),*b=[d stringByAppendingPathComponent:S("ThermalModel.original.plist")];if(!m||![NSFileManager.defaultManager fileExistsAtPath:b])return 78;[NSFileManager.defaultManager removeItemAtPath:m error:nil];if(![NSFileManager.defaultManager copyItemAtPath:b toPath:m error:nil])return 79;MakeStorageWritable(t);SetRuntimePerformancePreference(NO);return RestartThermalMonitor();}
